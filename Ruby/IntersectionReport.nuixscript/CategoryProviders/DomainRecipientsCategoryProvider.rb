class DomainRecipientsCategoryProvider < CategoryProviderBase
	def initialize
		super
		@domain_regex = /^.*@([^@]+)$/
	end
	...
	def label
		return "Domain - Recipients (within scope)"
	end
	...
	def named_queries(scope_query)
		...
		result = []
		result += distinct_domains.keys.map{|address| NamedQuery.new(address,"to-mail-domain:\"#{address}\" OR cc-mail-domain:\"#{address}\" OR bcc-mail-domain:\"#{address}\"")}
