class ItemDateFullCategoryProvider < CategoryProviderBase
	def label
		return "Item Date (YYYY/MM/DD, Within Scope)"
	end
	...
	def named_queries(scope_query)
		...
		scope_items.each do |item|
			...
			if !item_date.nil?
				...
				item_date_string = item_date.toString("YYYYMMdd")
				item_date_display_string = item_date.toString("YYYY/MM/dd")
		...
		distinct_dates.sort_by{|k,v|k}.each do |query_date,display_date|
			result << NamedQuery.new(display_date,"item-date:#{query_date}")
		end
