class ItemDateYearMonthNameCategoryProvider < CategoryProviderBase
	def label
		return "Item Date (YYYY MONTH, Within Scope)"
	end
	...
	def named_queries(scope_query)
		...
		scope_items.each do |item|
			...
			if !item_date.nil?
				...
				item_date_display_string = item_date.toString("YYYY MMMM")
